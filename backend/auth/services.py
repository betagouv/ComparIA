...
async def has_current_terms_acceptance(
    *, user_id: uuid.UUID | None, anonymous_user_hash: str
    ...
    """Return whether the actor accepted the currently published terms."""
...
async def erase_user_account(user_id: uuid.UUID) -> None:
    """Irreversibly detach account identifiers while preserving integrity.

    Conversation rows remain for integrity but are detached from the account.
    This cannot recall copies already exported to downstream datasets.
    """
    now = datetime.now()
    async with get_session() as session:
