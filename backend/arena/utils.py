...
def serialize_conversation_for_redis(conv: "Conversation") -> dict:
    """
    Serialize Pydantic Conversation model for Redis storage.

    Converts Conversation (runtime Pydantic model) to dict format for Redis.
    Handles Endpoint serialization and message type conversion.

    Args:
        conv: Conversation model to serialize

    Returns:
        dict: Serialized conversation data
    """
    return {
        "conv_id": conv.conv_id,
        "model_name": conv.model_name,
        "endpoint": conv.endpoint.model_dump() if conv.endpoint else None,
        "messages": [msg.model_dump() for msg in conv.messages],
    }


def deserialize_conversation_from_redis(data: dict) -> "Conversation":
    """
    Reconstruct Conversation from Redis storage.

    Parses message dicts back to proper message types based on role.

    Args:
        data: Dict from Redis with conversation data

    Returns:
        Conversation: Reconstructed Conversation model
    """
    ...
    endpoint = Endpoint(**endpoint_data) if endpoint_data else None
