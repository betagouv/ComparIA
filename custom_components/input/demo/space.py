...
```bash
pip install gradio_nostyleinput
```
...
""", elem_classes=["md-custom"], header_links=True)
    ...
    gr.ParamViewer(value=_docs["NoStyleInput"]["members"]["__init__"], linkify=[])


    gr.Markdown("### Events")
    gr.ParamViewer(value=_docs["NoStyleInput"]["events"], linkify=['Event'])
...
def predict(
    ...
    return value
```
""", elem_classes=["md-custom", "NoStyleInput-user-fn"], header_links=True)
