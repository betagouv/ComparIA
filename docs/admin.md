...
The app's secrets are encrypted in the database with `COMPARIA_ENCRYPTION_KEY`, which the backend refuses to start without outside debug mode. The same key protects the provider API keys of the endpoints, the moderation key and the publishing credentials. Generate one with:
