...
The app's secrets are encrypted in the database with `COMPARIA_ENCRYPTION_KEY`, which the backend refuses to start without outside debug mode. Generate one with:
