...
Uses Bradley-Terry model via arena-rank library.
"""
...
def get_battles_from_reactions(reactions: pl.DataFrame) -> pl.DataFrame:
    ...
    Liked reaction = vote for that model, disliked = vote for opponent.
    """
    with_signal = reactions.filter(pl.col("liked") | pl.col("disliked"))

    return with_signal.select([
        pl.col("model_a_name").alias("model_a"),
        pl.col("model_b_name").alias("model_b"),
        pl.when(
            pl.col("liked") & (pl.col("refers_to_model") == pl.col("model_a_name"))
        ).then(pl.lit("model_a"))
        .when(
            pl.col("liked") & (pl.col("refers_to_model") == pl.col("model_b_name"))
        ).then(pl.lit("model_b"))
        .when(
            pl.col("disliked") & (pl.col("refers_to_model") == pl.col("model_a_name"))
        ).then(pl.lit("model_b"))
        .when(
            pl.col("disliked") & (pl.col("refers_to_model") == pl.col("model_b_name"))
        ).then(pl.lit("model_a"))
...
def compute_rankings(votes: pl.DataFrame, reactions: pl.DataFrame) -> tuple[pl.DataFrame, pl.DataFrame]:
    ...
    log.info("Computing Bradley-Terry rankings...")

    vote_battles = get_battles_from_votes(votes)
    reaction_battles = get_battles_from_reactions(reactions)

    log.info(f"Battles: {len(vote_battles)} from votes, {len(reaction_battles)} from reactions")

    all_battles = pl.concat([vote_battles, reaction_battles])
    log.info(f"Total battles: {len(all_battles)}")
    ...
    log.info(f"Unique models: {dataset.n_competitors}")
...
def compute_rank_bounds(df: pl.DataFrame) -> pl.DataFrame:
    ...
    log.debug("Computing rank confidence bounds...")
...
def compute_match_counts(votes: pl.DataFrame, reactions: pl.DataFrame) -> pl.DataFrame:
    ...
    log.debug("Computing match counts...")
    ...
    reactions_with_signal = reactions.filter(pl.col("liked") | pl.col("disliked"))
...
def compute_mean_win_prob(rankings: pl.DataFrame) -> pl.DataFrame:
    """Compute mean win probability based on Bradley-Terry scores."""
    log.debug("Computing mean win probabilities...")
...
def compute_preferences(votes: pl.DataFrame) -> pl.DataFrame:
    ...
    log.debug("Computing preferences...")
...
def main():
    ...
    # Summary statistics
    models_with_prefs = len([m for m in output["models"] if m.get("total_prefs", 0) > 0])
    top_model = output["models"][0] if output["models"] else None

    log.info(f"Output written to {OUTPUT_PATH}")
    log.info(f"Summary: {len(result)} models ranked, {models_with_prefs} with preference data")
    if top_model:
        log.info(f"Top model: {top_model['model_name']} (ELO: {top_model['median']:.0f})")
