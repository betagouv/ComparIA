...
describe('Terms document administration', () => {
  ...
  it('allows an administrator to create the first document', async () => {
    mocks.request.mockImplementation((path: string) => {
      if (path === '/admin/legal/terms') return Promise.resolve([])
    ...
    await waitFor(() =>
      expect(getByRole('heading', { name: 'Aucune condition publiée' })).toBeTruthy()
    )
    await fireEvent.click(getByRole('button', { name: 'Créer les conditions d’utilisation' }))

    expect(getByRole('textbox', { name: /Contenu des conditions/ })).toBeTruthy()
  })

  it('publishes the new document without coupling it to the participation journey', async () => {
    const { getByRole } = render(TermsDocumentAdmin)
    await waitFor(() => expect(getByRole('heading', { name: 'Version 1.0' })).toBeTruthy())
    await fireEvent.click(getByRole('button', { name: 'Préparer une nouvelle version' }))
