...
<section class="max-w-[1000px]" aria-label="Conditions d’utilisation">
  ...
  {:else if !editorOpen}
    ...
    {:else}
      <div class="fr-alert fr-alert--warning fr-mb-4v" role="status">
        <h2 class="fr-alert__title">Aucune condition publiée</h2>
        <p>Publiez les conditions d’utilisation avant d’ouvrir l’arène au public.</p>
      </div>
      <Button text="Créer les conditions d’utilisation" onclick={startDraft} />
    {/if}
