...
const terms: ConsentDocument = {
  ...
  presentation: {
    ...
    signIn: {
      ...
      links: [{ label: 'Confidentialité', href: '/arene/donnees-personnelles' }]
    }
  }
}
...
describe('consent evidence', () => {
  ...
  it('keeps an explicit analytics refusal', () => {
    const storage = memoryStorage()
    setAnalyticsChoice('refused', storage)
    expect(storage.getItem(ANALYTICS_STORAGE_KEY)).toBe('refused')
    expect(getAnalyticsChoice(storage)).toBe('refused')
  })
})
