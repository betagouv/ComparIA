...
describe('TOSModal', () => {
        ...
        return Promise.resolve({
          ...
          presentation: {
            arena: {
              ...
              links: [{ label: 'Lire les conditions', href: '/arene/modalites' }],
              button_label: '**Valider** et envoyer'
            },
            sign_in: { checkbox_label: 'Je confirme.', links: [] }
          }
        })
      }
  ...
  it('uses the DSFR footer for a compact and coherent action area', async () => {
    const { container } = render(TOSModal)

    await waitFor(() => expect(mocks.request).toHaveBeenCalledTimes(2))

    const footer = container.querySelector('.fr-modal__footer')
    expect(footer).not.toBeNull()
    expect(footer?.querySelector('button')?.textContent).toContain('Valider et envoyer')
    expect(footer?.querySelector('button strong')?.textContent).toBe('Valider')
    expect(container.querySelector('.fr-modal__content > div > button')).toBeNull()
  })
})
