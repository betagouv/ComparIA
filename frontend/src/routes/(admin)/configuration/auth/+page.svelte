...
<div class="px-8 py-8 max-w-2xl">
  <div class="mb-8">
    ...
    <p class="fr-text--sm mb-0! text-[--text-mention-grey]">
      Etat actuel, modifiable via variables d'environnement.
    </p>
  </div>
            ...
            <td>
              {#if config.access_policy === 'sign_in_required'}
                ...
                <span class="fr-text--sm ml-2! text-[--text-mention-grey]"
                  >Connexion obligatoire pour accéder à l'arène</span
                >
              {:else}
                <Badge text="anonymous_first" variant="light-info" size="sm" />
                <span class="fr-text--sm ml-2! text-[--text-mention-grey]"
                  >Connexion facultative</span
                >
              {/if}
            </td>
          </tr>
          ...
          <tr>
            ...
            <td>
              ...
              {:else}
                ...
                <span class="fr-text--sm ml-2! text-[--text-mention-grey]"
                  >Les codes de connexion sont logués en console</span
                >
              {/if}
            </td>
          </tr>
            ...
            <td>
              ...
              {:else}
                <span class="fr-text--sm text-[--text-mention-grey]"
                  >Tous les domaines acceptés</span
                >
              {/if}
            </td>
          </tr>
